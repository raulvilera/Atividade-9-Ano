...
export default function Home() {
  ...
  return <main className="page-shell">
    ...
    <div className="activity-layout"><aside className="side-index"><div className="side-sticky"><div className="catalog-spine"><span>REGISTRO</span><b>9A / 03</b></div><span className="eyebrow">ÍNDICE DE EVIDÊNCIAS</span><p>Observe a imagem antes de responder. Nas questões dissertativas, escreva com suas próprias palavras e use conceitos científicos.</p><div className="index-list">{questions.map((question) => <a key={question.number} href={`#questao-${question.number}`} onClick={() => setActiveQuestion(question.number)} className={`${activeQuestion === question.number ? "active" : ""} ${answers[question.number] !== undefined || essays[question.number] ? "done" : ""}`} aria-current={activeQuestion === question.number ? "location" : undefined}><b>{String(question.number).padStart(2, "0")}</b><span>{question.type}</span></a>)}</div></div></aside>
